...
glEnd();
glBegin(GL_LINES);
glVertex2d(round((lineX1 * cos(theta)) - (lineY1 * sin(theta))), round((lineX1 *
sin(theta)) + (lineY1 * cos(theta))));
glVertex2d(round((lineX2 * cos(theta)) - (lineY2 * sin(theta))), round((lineX2 *
sin(theta)) + (lineY2 * cos(theta))));
glEnd();
}
void myInit(void)
{
